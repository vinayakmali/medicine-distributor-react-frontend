...
useEffect(()=>{
  ...
  const handleEditCellChange = React.useCallback(
    ({ id, field, props }) => {
      ...
      let store = {};
      if (field == 'name') { store = {id: id,name:props.value}; }
      if (field == 'location') { store = {id: id,location:props.value}; }
console.log(store)
      serv.updateStore(store).then((resp)=>{
        const status = resp.data.success;
    ...
    <Paper className={classes.root}>
      ...
      <div className={classes.pageHeading}>
	    	<Grid container>
	        	...
	        	<Grid item>
	        		<Link to="/addStore" color="inherit" style={linkStyle}>Add Store</Link>
	        	</Grid>
	      	</Grid>
	    </div>
    	<div className={classes.pageHeading}>
