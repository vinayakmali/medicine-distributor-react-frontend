...
useEffect(()=>{
  ...
  const handleEditCellChange = React.useCallback(
    ({ id, field, props }) => {
      ...
      let Medicines = {};
      if (field == 'name') { Medicines = {id: id,name:props.value}; }
      serv.updateMedicines(Medicines).then((resp)=>{
        const status = resp.data.success;
  ...
  return (
    ...
    <Paper className={classes.root}>
      ...
      <div className={classes.pageHeading}>
	    	<Grid container>
	        	<Grid item xs>
	          		<Typography component="h1" variant="h5"> Medicines List </Typography>
	        	</Grid>
	        	<Grid item>
	        		<Link to="/addMedicine" color="inherit" style={linkStyle}>Add New Medicines</Link>
	        	</Grid>
	      	</Grid>
	    </div>
    	<div className={classes.pageHeading}>
