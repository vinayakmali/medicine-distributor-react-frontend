...
const BarchartComponent=()=>{
	...
	useEffect(()=>{
		...
		//Report 1
		serv.getReports({type:'citywise'}).then((resp)=>{
		const newArr = [];			
	...
	const handleChange = (event) => {
        if(event.target.value == 'agent_wise'){
            			//Report 2
                serv.getReports({type:'agentwise'}).then((resp)=>{
                    const newArr = [];			
                        var res = Object.keys(resp.data.data).forEach(function(key) {
                            var obj = {};
                            Object.keys(resp.data.data[key]).forEach(function(keyArr) {
    ...
    if(event.target.value == 'return_medicine'){
			//Report 3
			serv.getReports({type:'returnMedicineReport'}).then((resp)=>{
    ...
    if(event.target.value == 'delivered_medicines'){
		//Report 4
		serv.getReports({type:'deliveredMedicineReport'}).then((resp)=>{
			const newArr = [];			
				var res = Object.keys(resp.data.data).forEach(function(key) {
					var obj = {};
					Object.keys(resp.data.data[key]).forEach(function(keyArr) {
    ...
    return(
        <div className="container">
			<Container component="main" maxWidth="xs">
                ...
                <InputLabel id="demo-controlled-open-select-label">Select Report Type</InputLabel>
                ...
                <MenuItem value='agent_wise'>Agentwise Delivery / Per Day)</MenuItem>
